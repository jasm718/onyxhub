...
function IsometricDeploymentScene() {
  ...
  return <div className="relative flex min-h-[320px] items-center justify-center lg:min-h-[380px]">
    <motion.div className="relative flex w-full max-w-[580px] items-center justify-center" initial={{ opacity: 0, scale: 0.92, y: 14 }} whileInView={{ opacity: 1, scale: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8 }}>
      <motion.div className="relative mx-auto w-full max-w-[500px]" animate={{ y: [0, -8, 0] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}>
  ...
  return <div ref={ref} className="relative overflow-hidden">
    <section className="relative bg-muted/30 py-24 lg:py-32">
      ...
      <div className="container relative z-10 mx-auto px-6 lg:px-12"><div className="grid items-center gap-14 lg:grid-cols-[0.85fr_1.15fr] lg:gap-16">
        ...
        <motion.div initial={{ opacity: 0, x: 36, scale: 0.97 }} animate={isInView ? { opacity: 1, x: 0, scale: 1 } : { opacity: 0, x: 36, scale: 0.97 }} transition={{ duration: 0.8, delay: 0.12 }} className="relative flex min-h-[420px] items-center justify-center sm:min-h-[520px] lg:min-h-[580px]"><div className="relative w-full max-w-[390px]"><div className="pointer-events-none absolute left-1/2 top-1/2 size-[72%] -translate-x-1/2 -translate-y-1/2 rounded-full bg-teal/10 blur-3xl" /><img src="/consultative-sales-amico.svg" alt="OnyxHub 成本优化方案插画" className="relative z-10 block h-auto w-full object-contain drop-shadow-xl" /></div></motion.div>
      </div></div>
    </section>

    <section className="border-t border-border/60 bg-background py-20 lg:py-28"><div className="container mx-auto px-6 lg:px-12"><div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-20"><motion.div initial={{ opacity: 0, x: -28 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-120px" }} transition={{ duration: 0.65 }}><div className="mb-5 flex size-12 items-center justify-center rounded-2xl bg-coral/10 text-coral"><Wrench className="size-6" /></div><h2 className="mb-5 text-3xl font-bold leading-tight text-foreground md:text-4xl">维护太<span className="text-coral">费力</span>？</h2><p className="mb-8 max-w-lg text-lg leading-relaxed text-muted-foreground">减少重复部署和逐台维护，让应用交付更简单，团队始终使用统一版本。</p><div className="grid gap-6 sm:grid-cols-2"><SolutionItem title="部署简单">一键安装、卸载，部署即用。</SolutionItem><SolutionItem title="版本统一">多用户使用统一版本应用。</SolutionItem></div></motion.div><motion.div initial={{ opacity: 0, x: 28 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-120px" }} transition={{ duration: 0.65, delay: 0.1 }}><IsometricDeploymentScene /></motion.div></div></div></section>

    <section className="border-t border-border/60 bg-muted/30 py-20 lg:py-28"><div className="container mx-auto px-6 lg:px-12"><div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-20"><motion.div initial={{ opacity: 0, x: -28 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-120px" }} transition={{ duration: 0.65 }} className="order-2 lg:order-1"><IsometricDataScene /></motion.div><motion.div initial={{ opacity: 0, x: 28 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-120px" }} transition={{ duration: 0.65, delay: 0.1 }} className="order-1 lg:order-2"><div className="mb-5 flex size-12 items-center justify-center rounded-2xl bg-teal/10 text-teal"><Database className="size-6" /></div><h2 className="mb-5 text-3xl font-bold leading-tight text-foreground md:text-4xl">数据有<span className="text-teal">泄漏</span>风险？</h2><p className="mb-8 max-w-lg text-lg leading-relaxed text-muted-foreground">让应用数据和业务文件回到统一边界，集中存储、隔离访问，减少终端数据暴露风险。</p><div className="space-y-6"><SolutionItem title="数据集中">应用数据、图纸文件全部存放于应用服务器上。</SolutionItem><SolutionItem title="数据不落地">服务器数据不与客户端数据互通。</SolutionItem><SolutionItem title="数据分隔">用户数据相互隔离，彼此不互通。</SolutionItem></div></motion.div></div></div></section>
  </div>
}
